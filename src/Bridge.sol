...
contract Bridge {
    ...
    function BridgeOut(address _erc20Contract, uint256 amount) public {
        ...
        withdrawBalance[msg.sender][_erc20Contract].startTime = block.timestamp;
    }

    function withdrawERC20(address _erc20Contract, uint256 amount) public {
        require(
            block.timestamp >=
                withdrawBalance[msg.sender][_erc20Contract].startTime +
                    delayTime,
            "Time doesn't pass enough yet"
        );
        require(
            withdrawBalance[msg.sender][_erc20Contract].amt >= amount,
